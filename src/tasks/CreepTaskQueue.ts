...
export class CreepTaskQueue {
  static removeFinished(): void {
    ...
    for (var id in _.filter(Memory.creepTasks, req => req.status == "FINISHED")) {
      this.removeTask(id)
    }
  }
  ...
  static removeTask(id: string): void {
    ...
    const creep = <Creep>Game.getObjectById(Memory.creepTasks[id].assignedToID);
  ...
  static getTask(id: string): CreepTaskRequest | undefined {
    ...
    const request = Memory.creepTasks[id];
    if (request == undefined) {
