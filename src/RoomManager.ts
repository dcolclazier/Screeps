...
export class RoomManager {
  ...
  public idleStructureIDs(roomName: string, refresh: boolean = false): string[] {
  ...
  public sources(roomName: string, refresh: boolean = false): SourceMemory[] {
    const room = Memory.rooms[roomName];
    if (room == undefined) {
      console.log("ERROR_getSources2 - undefined room in memory? how? " + roomName);
      return [];
    }
    if (this._sources2[roomName] == undefined || refresh) {
  ...
  public containers(roomName: string, refresh: boolean = false): ContainerMemory[] {
    const room = Memory.rooms[roomName];
    if (room == undefined) {
      console.log("ERROR_getContainers2 - undefined room in memory? how? " + roomName);
      return [];
    }
    if (this._containers2[roomName] == undefined || refresh) {
  ...
  public links(roomName: string, refresh: boolean = false): LinkMemory[] {
    const room = Memory.rooms[roomName];
    if (room == undefined) {
    ...
    if (links == undefined || refresh) {
      links = this._links2[roomName] = this.loadLinks2(roomName);
    }
    return links;
  }
  public towers(roomName: string, refresh: boolean = false): TowerMemory[] {
    const room = Memory.rooms[roomName];
    if (room == undefined) {
      console.log("ERROR_getLinks2 - undefined room in memory? how?" + roomName);
      return [];
